...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.SpinBox_bp_price_min.setMaximum(999999999)
        ...
        self.TableWidget_search_result.setColumnCount(13)
        ...
        item = QtWidgets.QTableWidgetItem()
        self.TableWidget_search_result.setHorizontalHeaderItem(12, item)
    ...
    def retranslateUi(self, MainWindow):
        ...
        item = self.TableWidget_search_result.horizontalHeaderItem(12)
    ...
    def event_handler(self):
        ...
        self.Button_excel.clicked.connect(self.excel_down)
    ...
    def crawler(self):
        totalCount = 0
        #self.TableWidget_search_result.clear()
        self.TableWidget_search_result.setRowCount(0)
        self.lcdNumber_search_total.display(0)
        self.progressBar.setValue(0)
        self.progressBar.setProperty("format", "Searching...  Season 0/"+str(self.ListWidget_season.count()))
        QtWidgets.QApplication.processEvents()
        #self.progressBar.setFormat("Searching...")
        select_bp = int(self.ComboBox_bp.currentData())
                ...
                for q in range(self.ListWidget_ovr.count()):
                    ...
                    for tr in t :
                        if tr.find('div', {'class':'name'}) : 
                            ...
                            season = self.ListWidget_season.item(i).text()
                            league = self.ListWidget_league.item(j).text()
                            
                            if   select_bp == 1:
                                bp_num = int(bp_1.replace(',',''))
                                if bp_num < self.SpinBox_bp_price_min.value(): continue
                            elif select_bp == 2:
                                bp_num = int(bp_2.replace(',',''))
                                if bp_num < self.SpinBox_bp_price_min.value(): continue
                            elif select_bp == 3:
                                bp_num = int(bp_3.replace(',',''))
                                if bp_num < self.SpinBox_bp_price_min.value(): continue
                            elif select_bp == 4:
                                bp_num = int(bp_4.replace(',',''))
                                if bp_num < self.SpinBox_bp_price_min.value(): continue
                            elif select_bp == 5:
                                bp_num = int(bp_5.replace(',',''))
                                if bp_num < self.SpinBox_bp_price_min.value(): continue
                            elif select_bp == 6:
                                bp_num = int(bp_6.replace(',',''))
                                if bp_num < self.SpinBox_bp_price_min.value(): continue
                            elif select_bp == 7:
                                bp_num = int(bp_7.replace(',',''))
                                if bp_num < self.SpinBox_bp_price_min.value(): continue
                            elif select_bp == 8:
                                bp_num = int(bp_8.replace(',',''))
                                if bp_num < self.SpinBox_bp_price_min.value(): continue
                            ...
                            item = QtWidgets.QTableWidgetItem(league)     ;item.setTextAlignment(QtCore.Qt.AlignCenter); self.TableWidget_search_result.setItem( row , 2, item)
                            item = QtWidgets.QTableWidgetItem(position)   ;item.setTextAlignment(QtCore.Qt.AlignCenter); self.TableWidget_search_result.setItem( row , 3, item)
                            item = QtWidgets.QTableWidgetItem(ovr)        ;item.setTextAlignment(QtCore.Qt.AlignCenter); self.TableWidget_search_result.setItem( row , 4, item)
                            item = QtWidgets.QTableWidgetItem(bp_1)       ;item.setTextAlignment(QtCore.Qt.AlignRight) ; self.TableWidget_search_result.setItem( row , 5, item)
                            item = QtWidgets.QTableWidgetItem(bp_2)       ;item.setTextAlignment(QtCore.Qt.AlignRight) ; self.TableWidget_search_result.setItem( row , 6, item)
                            item = QtWidgets.QTableWidgetItem(bp_3)       ;item.setTextAlignment(QtCore.Qt.AlignRight) ; self.TableWidget_search_result.setItem( row , 7, item)
                            item = QtWidgets.QTableWidgetItem(bp_4)       ;item.setTextAlignment(QtCore.Qt.AlignRight) ; self.TableWidget_search_result.setItem( row , 8, item)
                            item = QtWidgets.QTableWidgetItem(bp_5)       ;item.setTextAlignment(QtCore.Qt.AlignRight) ; self.TableWidget_search_result.setItem( row , 9, item)
                            item = QtWidgets.QTableWidgetItem(bp_6)       ;item.setTextAlignment(QtCore.Qt.AlignRight) ; self.TableWidget_search_result.setItem( row ,10, item)
                            item = QtWidgets.QTableWidgetItem(bp_7)       ;item.setTextAlignment(QtCore.Qt.AlignRight) ; self.TableWidget_search_result.setItem( row ,11, item)
                            item = QtWidgets.QTableWidgetItem(bp_8)       ;item.setTextAlignment(QtCore.Qt.AlignRight) ; self.TableWidget_search_result.setItem( row ,12, item)
                            totalCount+=1
            self.progressBar.setProperty("value", ( (i+1) / self.ListWidget_season.count() )*100)
            self.progressBar.setProperty("format", "Searching...  Season "+str((i+1))+"/"+str(self.ListWidget_season.count()))
            QtWidgets.QApplication.processEvents()
        self.progressBar.setProperty("format", "Search Success !! ")
        print("##### END CRAWLER ")
    ...
    def save_env(self):
        data = dict()
        list_season = list(); list_league = list(); list_position = list(); list_ovr = list()

        for i in range( self.ListWidget_season.count() ) :
            temp = dict()
            temp["val"] = self.ListWidget_season.item(i).data(QtCore.Qt.UserRole)
            temp["txt"] = self.ListWidget_season.item(i).text()
            list_season.append(temp)
        for i in range( self.ListWidget_league.count() ) :
            temp = dict()
            temp["val"] = self.ListWidget_league.item(i).data(QtCore.Qt.UserRole)
            temp["txt"] = self.ListWidget_league.item(i).text()
            list_league.append(temp)
        for i in range( self.ListWidget_position.count() ) :
            temp = dict()
            temp["val"] = self.ListWidget_position.item(i).data(QtCore.Qt.UserRole)
            temp["txt"] = self.ListWidget_position.item(i).text()
            list_position.append(temp)
        for i in range( self.ListWidget_ovr.count() ) :
            temp = dict()
            temp["val"] = self.ListWidget_ovr.item(i).data(QtCore.Qt.UserRole)
            temp["txt"] = self.ListWidget_ovr.item(i).text()
            list_ovr.append(temp)
        
        data["LIST_SEASON"]    = list_season
        data["LIST_LEGUE"]     = list_league
        data["LIST_POSITION"]  = list_position
        data["LIST_OVR"]       = list_ovr

        with open('config\\Environment.json','w',encoding='utf-8') as make_file:
            json.dump(data, make_file, indent="\t")
            
    def load_env(self):
        with open('config\\Environment.json') as env_file:
            env_data = json.load(env_file)
            list_season   = env_data['LIST_SEASON']
            list_league   = env_data["LIST_LEGUE"]   
            list_position = env_data["LIST_POSITION"]
            list_ovr      = env_data["LIST_OVR"] 

            for i in range(len(list(list_season))):
                list_item = QtWidgets.QListWidgetItem()
                list_item.setData(QtCore.Qt.UserRole , list_season[i]['val'] )
                list_item.setText( list_season[i]['txt'] )
                self.ListWidget_season.addItem(list_item)
            for i in range(len(list(list_league))):
                list_item = QtWidgets.QListWidgetItem()
                list_item.setData(QtCore.Qt.UserRole , list_league[i]['val'] )
                list_item.setText( list_league[i]['txt'] )
                self.ListWidget_league.addItem(list_item)
            for i in range(len(list(list_position))):
                list_item = QtWidgets.QListWidgetItem()
                list_item.setData(QtCore.Qt.UserRole , list_position[i]['val'] )
                list_item.setText( list_position[i]['txt'] )
                self.ListWidget_position.addItem(list_item)
            for i in range(len(list(list_ovr))):
                list_item = QtWidgets.QListWidgetItem()
                list_item.setData(QtCore.Qt.UserRole , list_ovr[i]['val'] )
                list_item.setText( list_ovr[i]['txt'] )
                self.ListWidget_ovr.addItem(list_item)

    def excel_down(self):
        excel_data = dict()
        for column in range( self.TableWidget_search_result.columnCount() ):
            list_data = list()
            columnName = self.TableWidget_search_result.horizontalHeaderItem(column).text()
            for row in range( self.TableWidget_search_result.rowCount() ):
                # if columnName == 'Bp.1' or columnName == 'Bp.2' or columnName == 'Bp.3' or columnName == 'Bp.4' or columnName == 'Bp.5' or columnName == 'Bp.6' or columnName == 'Bp.7' or columnName == 'Bp.8' or columnName == 'Ovr' :
                #     item = int(self.TableWidget_search_result.item(row,column).text().replace(",",""))
                # else: 
                item = self.TableWidget_search_result.item(row,column).text()
                list_data.append(item)
            excel_data[columnName] = list_data
        
        excel_data = pd.DataFrame(excel_data)
        excel_data.to_excel(excel_writer='transfer_info.xlsx')
